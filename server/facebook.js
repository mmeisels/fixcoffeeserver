            ...
            db.query('SELECT id, firstName, lastName, email, loyaltyid__c as externalUserId FROM salesforce.contact WHERE fbUserId__c=$1', [fbUser.id], true)
                .then(function (user) {
                    if (user) {
                        ...
                        winston.info('Not a Facebook user');
...
function updateUser(user, fbUserId) {
   ...
   var externalUserId = (+new Date()).toString(36); // TODO: more robust UID logic
     var pictureURL = 'https://graph.facebook.com/' +fbUserId + '/picture?width=140&height=140';
    winston.info("Picture user " + pictureURL);
   return db.query(
        'UPDATE salesforce.contact SET fbUserId__c=$1, loyaltyid__c=$2, pictureURL__c=$4 WHERE id=$3 RETURNING id, firstName, lastName, email, loyaltyid__c as externalUserId',
        [fbUserId, externalUserId, user.id,pictureURL], true);
}
